...
从[Swoole官方文档](https://wiki.swoole.com)角度来看Swoole虽然是标准的PHP扩展，但实际上又与普通扩展只提供一个库函数不同，它所做的是接管PHP的控制权，进入事件循环。当IO事件发生后底层会自动回调指定的PHP函数。swoole是php的异步并行，高性能网络通信引擎，使用纯c语音编写，提供了php语言的异步多线程服务器。
